######################################################################
# script to make the plot and the table for the high dimensional model selection sim
######################################################################


# load in results from the simulation study

rm(list=ls())

modelRes <- readRDS('./modelselectionResults.rds')
...
ggplot(frameForPlot, aes(x=commonDistinctive, y=MSE, shape = percentageOfZeroes)) + 
    ...
    ggsave('allComDisPlot.pdf', dpi=300)


system(paste("pdfcrop", 'allComDisPlot.pdf', 'allComDisPlot.pdf'))
